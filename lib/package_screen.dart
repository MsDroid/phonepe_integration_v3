...
class _PaymentPageState extends State<PaymentPage> {
  ...
  Future<void> initiatePhonePePayment(Map<String, String> formData) async {
    const String backendUrl = 'https://dortoservices.in/phonepay';
    ...
    try {
      ...
      final response = await http.get(Uri.parse(backendUrl), headers: {'Content-Type': 'application/json'});
      print(response.statusCode);

      if (response.statusCode == 200) {
        final responseData = jsonDecode(response.body);
        print(responseData);
        final String merchantId = 'M23MD8JKBY7H7UAT';

        /// responseData['merchantId'];
        final String flowId = "DOOR12353"; // responseData['flowId'];
                ...
                : Form(
                  ...
                  child: ListView(
                    children: [
                      ...
                      ElevatedButton(onPressed: _onPayNow, child: const Text("Pay Now")),
                      if (_paymentStatus != null) ...[const SizedBox(height: 20), Text(_paymentStatus!, style: const TextStyle(color: Colors.green))],
                    ],
                  ),
                ),
      ),
